...
public class Argo1StoreEng extends StoreEngine  {
     ...
     */
    public Argo1StoreEng (int memory_size_in_bytes)
    {
	...
	*/
    ...
    public HashMap<Integer, HashMap<String, String>> selectWhereSingle(byte[][] selectCols, byte[] whereCol, String relation, byte[] value){
        ...
        // start from the beginning 
		readBuf.position(0);
		byte [] key,valstr,conditionStr = value, valbool = new byte[1];
		long valnum=0;
		...
		while(readBuf.position()<bound){ 
			...
			if(conditionFlag == 2){
				readBuf.position(readBuf.position() + 8 + 8 + 8 + 1);
				continue;
				...
				if(isSelected(selectCols, key) == true){ //select this field
					pos = (int)readBuf.getLong();
					if(pos == UNDEFINED){
						//TODO: also support double nums
						//if pos is UNDEFINED, then it is either LONG or BOOL
						valnum = readBuf.getLong();
						if(valnum == UNDEFINED){
					...
					readBuf.position(readBuf.position() + 8 + 8 + 1);	
				continue;
			}
			//this field has not yet been checked:
			//read the key
			int pos = (int)readBuf.getLong();
			stringReadBuf.position(pos);
			len = stringReadBuf.getInt();
			...
			if(Arrays.equals(key, whereCol)==true){
				pos = (int)readBuf.getLong(); 
				if(pos == UNDEFINED){
					readBuf.position(readBuf.position() + 8 + 1);
					continue;
				}	
				stringReadBuf.position(pos);
				len = stringReadBuf.getInt();
				...
				//if the condition is met, position to the start of the object
				if(conditionFlag == 1)
